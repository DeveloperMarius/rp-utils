...
class Security{
     ...
     * @var string
     */
    public static string $hashType = 'sha256';//'sha1';
     ...
     * @var OpenSsl|null
     */
    protected static ?OpenSsl $_instance;
     ...
     */
    public static function hash(string $string, ?string $algorithm = null, bool|string $salt = false): string{
        if(empty($algorithm)){
    ...
    public static function insecureRandomBytes(int $length): string{
        ...
        while($byteLength < $length){
            $bytes .= static::hash(Util::generateUuid() . uniqid((string)mt_rand(), true), 'sha512', true);
     ...
     *
     * @param OpenSsl|null $instance The crypto instance to use.
     * @return OpenSsl Crypto instance.
     * @throws \InvalidArgumentException When no compatible crypto extension is available.
     * @psalm-suppress MoreSpecificReturnType
     */
    public static function engine(OpenSsl $instance = null){
        if($instance){
     ...
     * @return bool
     */
    public static function checkHash(string $string, string $hash, ?string $algorithm = null, bool|string $salt = false): bool{
        return self::constantEquals(self::hash($string, $algorithm, $salt), $hash);
    }
